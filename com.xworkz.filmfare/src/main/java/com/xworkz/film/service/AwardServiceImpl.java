	...
	public boolean validateAndSave(AwardDTO awardDTO) {
		...
		try {
			...
			if(Objects.nonNull(awardDTO)) {
				...
				String spansar = awardDTO.getSpansar();
				if (spansar !=null && !spansar.isEmpty() && spansar.length() >=5) {
					System.out.println("spansar is valid");
					valid = true;	
			} else {
				if(valid) {
					System.out.println("spansar is invalid");
					valid = false;
